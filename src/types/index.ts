...
export function transformLegacyToGrounding(
  ...
  // Create Landing AI-style grounding box (l, t, r, b format)
  const normalizedBox: GroundingBox = {
    l: Math.max(0, Math.min(legacyChunk.geometry.x / pageWidth, 1)), // Convert pixels to normalized
    t: Math.max(0, Math.min(legacyChunk.geometry.y / pageHeight, 1)), // Convert pixels to normalized
    r: Math.max(0, Math.min((legacyChunk.geometry.x + legacyChunk.geometry.w) / pageWidth, 1)), // Convert pixels to normalized
    b: Math.max(0, Math.min((legacyChunk.geometry.y + legacyChunk.geometry.h) / pageHeight, 1)), // Convert pixels to normalized
  };
