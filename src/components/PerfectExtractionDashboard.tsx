...
import { ExtractionResult, Block, Line as TextLine, Word, BoundingBox } from "@/types";
...
export default function PerfectExtractionDashboard({
  ...
  const calculateMetrics = (): MetricCard[] => {
    ...
    // Elements detected
    const totalElements = (extractionResult.chunks?.length || 0) + 
  ...
  const calculateAverageConfidence = (): number => {
    ...
    // Text chunks confidences
    if (extractionResult.chunks) {
      extractionResult.chunks.forEach(chunk => {
        if (chunk.confidence) {
          totalConfidence += chunk.confidence * 100;
          count++;
        }
      });
    }
  ...
  const getLayoutElements = (): LayoutElement[] => {
    ...
    if (page.lines && page.lines.length > 0) {
      const avgConfidence = page.lines.reduce((sum: number, l: TextLine) => sum + (l.words.reduce((wordSum: number, w: Word) => wordSum + w.confidence, 0) / l.words.length || 0), 0) / page.lines.length;
      elements.push({
        type: "Lines",
    ...
    if (page.semanticRegions && page.semanticRegions.length > 0) {
      const avgConfidence = page.semanticRegions.reduce((sum: number, s) => sum + (s.confidence || 0), 0) / page.semanticRegions.length;
      elements.push({
                  ...
                  <CardContent className="space-y-3">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-mono">
                          {extractionResult.chunks?.length || 0}
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
